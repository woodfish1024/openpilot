...
def get_radar_can_parser(CP, radar_tracks, escc, msg_start_addr, msg_count, radar_group4=False):
  if escc:
    lead_src, bus = "ESCC", 0
...
class RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    self.rcp_tracks = get_radar_can_parser(CP, self.radar_tracks, self.enhanced_scc, self.radar_start_addr, self.radar_msg_count, self.radar_group4)
  ...
  def update(self, can_strings):
    ...
    scc_ready = not self.radar_tracks and not self.enhanced_scc and self.frame % 5 == 0 and self.rcp_scc is not None
    ...
    if ((self.rcp_tracks is not None and (self.radar_tracks or self.enhanced_scc) and not self.rcp_tracks.can_valid) or
